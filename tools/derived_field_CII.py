...
solar_metallicity = 0.0134 # asplund et.al 2009
...
#     return data["HI density"].to('g/cm**3').copy()
...
#     return data["HII density"].to('g/cm**3').copy()
...
#     return data["H2 density"].to('g/cm**3').copy()
...
#     return data["HeI density"].to('g/cm**3').copy()
...
#     return data["HeII density"].to('g/cm**3').copy()
...
#     return data["HeIII density"].to('g/cm**3').copy()
...
def _test_mass(field, data):
    return data["H_density"].to('g/cm**3')/data["H_density"].to('g/cm**3')*(1.007825032*amu)
yt.add_field(("gas", "test_mass"), function=_test_mass, units="g",sampling_type="local")

def _HI_number_density(field, data):
    return data["H_density"].to('g/cm**3')/(H_mass)
...
def _HII_number_density(field, data):
    return data["H_p1_density"].to('g/cm**3')/(H_mass-electron_mass)
...
def _H2_number_density(field, data):
    return data["H2_density"].to('g/cm**3')/(H2_mass)
...
def _HeI_number_density(field, data):
    return data["He_density"].to('g/cm**3')/(He_mass)
...
def _HeII_number_density(field, data):
    return data["He_p1_density"].to('g/cm**3')/(He_mass-electron_mass)
...
def _HeIII_number_density(field, data):
    return data["He_p2_density"].to('g/cm**3')/(He_mass-2*electron_mass)
...
def _log_dust_attenuation(field, data):
    return -(data["H_number_density"].to('1/cm**3')+2*data["H2_number_density"].to('1/cm**3'))*data['metallicity']*solar_metallicity*L_soblen*dust_cross_section # With H2
#    return -(data["HI number density"].to('g/cm**3'))*data['metallicity']*solar_metallicity*L_soblen*dust_cross_section # Without H2
yt.add_field(("gas", "log_dust_attenuation"), function=_log_dust_attenuation, units="",sampling_type="local")

def _rCIIe(field, data):
    return 6.67*10**(-20)*Kelvin**(0.5)*np.exp(-91.2*Kelvin/data['temperature'].to('K'))/data["temperature"].to('K')**0.5*erg*centimeter**3/second #Not sure which one contains the per second and why there is a square root of temperature
yt.add_field(("gas", "rCIIe"), function=_rCIIe, units="erg*cm**3/s",sampling_type="local")

def _rCIIa(field, data):
    x = 16 + .344*(data['temperature'].to('K')/Kelvin)**(0.5) - 47.7*Kelvin/data['temperature'].to('K')
    x[x<0.0]=0
    return 10**(-24)*np.exp(-91.2*Kelvin/data['temperature'].to('K'))*x*erg*centimeter**3/second #Not sure which one contains the per second and why there is a square root of temperature                
yt.add_field(("gas", "rCIIa"), function=_rCIIe, units="erg*cm**3/s",sampling_type="local")

def _CII_e_cooling(field, data):
    return data['rCIIe'] * (data['H_p1_number_density'].to('1/cm**3')+data['He_p1_number_density'].to('1/cm**3')+2*data['He_p2_number_density'].to('1/cm**3')) * CII_abun * data['H_number_density'].to('1/cm**3') * data['metallicity']*solar_metallicity
yt.add_field(("gas", "CII_e_cooling"), function=_CII_e_cooling, units="erg/cm**3/s",sampling_type="local")

def _CII_a_cooling(field, data):
    return data['rCIIa'] * CII_abun * data['H_number_density'].to('1/cm**3') * data['H_number_density'].to('1/cm**3') * data['metallicity']*solar_metallicity
...
def _CII_HeI_cooling(field, data):
    return 0.38 * data['rCIIa'] * data['He_number_density'].to('1/cm**3') * CII_abun * data['H_number_density'].to('1/cm**3') * data['metallicity']*solar_metallicity
...
def _CII_CMB_emission(field, data):
    ...
    return 2.0 *  CII_abun * data['H_number_density'].to('1/cm**3') * data['metallicity']*solar_metallicity*2.298*10**(-6)*kboltz*91.2*Kelvin*np.exp(-91.2*Kelvin/(2.725*Kelvin)*1/(1+z))/second # not sure which one contains the per second unit
...
def _CII_H2_para(field, data):
    ...
    """
    rate_coefficient = 4.25*10**(-10)*centimeter**3/second*unyt_array(pow(data['temperature'].to('K').to_ndarray()/(100),0.124-0.018*np.log(data['temperature'].to('K').to_ndarray()/(100))))# Draine Table F6 pg 501
    return rate_coefficient*kboltz*(91.2*Kelvin)*CII_abun*data['H_number_density'].to('1/cm**3')*0.25*data['H2_number_density'].to('1/cm**3')
...
def _CII_H2_ortho(field, data):
    ...
    These are cooling rates, i.e. Lambda*n^2 where Lambda is the cooling function.. See https://www.astro.umd.edu/~richard/ASTRO620/A620_2015_Gas_lec2.pdf                 
    """
    rate_coefficient = (5.14*10**(-10)*centimeter**3/second)*unyt_array(pow(data['temperature'].to('K').to_ndarray()/(100),0.095+0.023*np.log(data['temperature'].to('K').to_ndarray()/(100))))# Draine Table F6 pg 501
    return rate_coefficient*kboltz*91.2*Kelvin*CII_abun*data['H_number_density'].to('1/cm**3')*0.75*data['H2_number_density'].to('1/cm**3')
...
#     CII Point Luminosity of electrons
#     """
#     return data['CII_H2_ortho']*0.75*(data['H2_number_density'].to('1/cm**3'))#*data['cell_volume'].to('cm**3')
...
#     CII Point Luminosity of electrons
#     """
#     return data['CII_H2_para']*0.25*(data['H2_number_density'].to('1/cm**3'))#*data['cell_volume'].to('cm**3')
...
#     return data['LCII_e']+data['LCII_a']+data['LCII_HeI']+data['LCII_CMB']+data['LCII_H2_ortho']+data['LCII_H2_para']
    return data['CII_e_cooling']+data['CII_a_cooling']+data['CII_HeI_cooling']+data['CII_CMB_emission']+data['CII_H2_ortho']+data['CII_H2_para']
